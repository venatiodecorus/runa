...
## 18. Groups (recorded direction — builds on M5, not yet scheduled)

**Framing decision (binding even now): groups are an audience primitive, never a trust primitive.** A group is an orthogonal membership set layered over the follow graph: it determines who can decrypt and see a space, while each viewer's own web (§3) still ranks content *within* it — members you follow rank higher; fellow members outside your web are visible because joining the group was your deliberate act. The trust math, constants, and 2-hop computation are unchanged by groups. Two explicit guards, because they're the tempting shortcuts: **group co-membership is never an input to trust** (§1.9 — joining a group is a deliberate act but not a vouch for every other member; a 10k-member group must not mint a trust clique), and co-membership never opens the reciprocal DM window (§5.1 — actual engagement, e.g. a reply to your group post, does, and the existing rule already covers it).

### 18.1 Private groups (the main case)

- **A private group = tier-3 with an explicit shared roster instead of a graph-derived, per-sender scope.** Same epoch-key mechanism as §7.1(3): random symmetric epoch key wrapped per member *device* over tier-2, rotate on membership change + max age. Consequently M5 must spec the epoch recipient set as an **abstract source** — graph-derived scope now, explicit roster later — so the mechanism is built once and groups aren't foreclosed.
- **Group identity = keypair**, mirroring accounts: group ID = base64url(group root pubkey), generated client-side by the creator. The group root signs admin certs (same pattern and verification code as device certs); admins sign membership records. Root custody mirrors account custody (exportable recovery kit); losing the group root freezes membership changes while existing members keep reading — a stated failure mode, not a surprise. Group identity is instance-independent like account identity (§15 — doesn't foreclose federation).
- **Membership visibility:** the roster is server-visible (routing and read-enforcement need it, and the tier-2 epoch fan-out reveals it to the operator anyway — consistent with §8's metadata concession) and **never served to non-members**; a private group's very existence is 403/404 to non-members. Same visibility tier as mutes and inbound follower lists. The threat model gains a line under A1 when this lands.
- **History semantics:** a per-group, creation-time choice — **from-join-onward** (default; the §7.2 snapshot semantics) or **history-visible** (an admitting member's client re-wraps past epoch keys to the new member's devices over tier-2 — a client-side act; the server never touches keys, §1.3 holds).
- **Moderation = curation, never deletion** (§1.4 holds): a signed group-remove-post record removes a post from the *group view* only — the record survives on the author's own timeline. Removing a member rotates the epoch (they receive nothing new); it never touches what they already decrypted, their standing, or their chosen edges.
- **Epoch liveness — the real engineering lift:** the server can't distribute keys, so membership changes need a live client. Removal → **lazy rotation**: the next poster generates the new epoch excluding the removed member (stated race: a member who hasn't yet seen the removal may post once more into the old epoch). Admission and re-enrolled devices (browser storage is disposable, §2.3) → **any current member's** client may wrap the current epoch key for the new device, spreading the availability burden beyond admins.
- **Budgets & reports compose unchanged:** inviting a stranger to a group is a cold initiation (one token, request tray, §5); posting within a group is warm by definition (everyone there chose the space). Reports on group content use the existing recipient plaintext-forwarding proof (§4.1) — a member is a legitimate, envelope-proven recipient.

### 18.2 Public groups (cheap; the imageboard's boards)

No crypto at all: a signed group record (name, description, open vs member-gated posting), tier-1 posts carrying a group reference, and the same curation record. **Pull, not push:** reading a public group is a deliberate act like explore mode — group posts never land in anyone's feed unprompted unless the author is already in the viewer's web. Posting to a public group is therefore unmetered, like posting to your own feed (§5.1). The viewer's own trust math may rank within the group view (or raw chronological, per instance/client preference). On imageboard-mode instances (§17), public groups render naturally as boards.

### 18.3 Sequencing

Do not build groups before M5 — build them as its extension: graph-scoped tier-3 posts first, then the roster/admin layer on the same epoch machinery. Public groups are crypto-independent and can land any time (natural pairing: M8 explore/discovery — both are discovery surfaces). New record types (group, group-admin-cert, group-member-add/-remove, group-invite, group posts carrying group + epoch id) are specced in the protocol doc with vectors before implementation, per the standing rules. No new trust constants → no simlab gate, though a group-Sybil scenario is a cheap sanity check.
